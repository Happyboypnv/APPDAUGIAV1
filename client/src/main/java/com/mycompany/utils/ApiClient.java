...
public class ApiClient {
    ...
    public static List<TransactionHistoryItemDTO> getTransactionHistory(String token) {
        String responseJson = guiGet("/api/transactions/me", token);
        if (responseJson == null) return new java.util.ArrayList<>();
        java.lang.reflect.Type listType =
                new com.google.gson.reflect.TypeToken<List<TransactionHistoryItemDTO>>(){}.getType();
        try {
            return gson.fromJson(responseJson, listType);
        } catch (Exception e) {
            logger.error("[ApiClient] Loi parse transaction history: " + e.getMessage());
            return new java.util.ArrayList<>();
        }
    }
     ...
     * @deprecated Dùng deposit() hoặc withdraw() thay thế để tránh race condition.
     */
    @Deprecated
    ...
    // ============================================================
    // NẠP TIỀN / RÚT TIỀN (delta-based — tránh race condition)
    // ============================================================

    /**
     * Nạp tiền: gọi POST /api/users/deposit với delta amount.
     * Server tự cộng vào DB → không bao giờ ghi đè số dư người khác.
     *
     * @param amount  số tiền cần nạp (phải > 0)
     * @param token   token đăng nhập
     * @return số dư mới sau khi nạp, hoặc -1 nếu thất bại
     */
    public static double deposit(double amount, String token) {
        try {
            String jsonBody = "{\"amount\":" + amount + "}";
            String responseJson = guiPost("/api/users/deposit", jsonBody, token);
            if (responseJson == null) return -1;
            com.google.gson.JsonObject resp = gson.fromJson(responseJson, com.google.gson.JsonObject.class);
            return resp != null && resp.has("balance") ? resp.get("balance").getAsDouble() : -1;
        } catch (Exception e) {
            logger.error("[ApiClient] Lỗi deposit: " + e.getMessage());
            return -1;
        }
    }

    /**
     * Rút tiền: gọi POST /api/users/withdraw với delta amount.
     * Server kiểm tra available_balance và trừ atomically.
     *
     * @param amount  số tiền cần rút (phải > 0)
     * @param token   token đăng nhập
     * @return số dư mới sau khi rút, hoặc -1 nếu không đủ tiền / lỗi
     */
    public static double withdraw(double amount, String token) {
        try {
            String jsonBody = "{\"amount\":" + amount + "}";
            String responseJson = guiPost("/api/users/withdraw", jsonBody, token);
            if (responseJson == null) return -1;
            com.google.gson.JsonObject resp = gson.fromJson(responseJson, com.google.gson.JsonObject.class);
            return resp != null && resp.has("balance") ? resp.get("balance").getAsDouble() : -1;
        } catch (Exception e) {
            logger.error("[ApiClient] Lỗi withdraw: " + e.getMessage());
            return -1;
        }
    }

    /**
     * Lấy số dư hiện tại của user từ server.
     *
     * @param token token đăng nhập
     * @return số dư thực tế, hoặc -1 nếu lỗi
     */
    public static double getBalance(String token) {
        try {
            String responseJson = guiGet("/api/users/me/balance", token);
            if (responseJson == null) return -1;
            com.google.gson.JsonObject resp = gson.fromJson(responseJson, com.google.gson.JsonObject.class);
            return resp != null && resp.has("balance") ? resp.get("balance").getAsDouble() : -1;
        } catch (Exception e) {
