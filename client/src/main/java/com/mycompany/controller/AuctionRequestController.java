...
public class AuctionRequestController implements Initializable {
    ...
    private void doRefresh() {
        new Thread(() -> {
            ...
            // Filter to show only PENDING auctions for admin review
            List<PhienDauGiaDTO> pendingList = new java.util.ArrayList<>();
            for (PhienDauGiaDTO dto : allAuctions) {
                if ("PENDING".equals(dto.trangThai)) {
                    pendingList.add(dto);
                }
            }
            Platform.runLater(() -> {
                danhSachPhien = pendingList;
                // Giữ vị trí scroll hiện tại
                int selectedIndex = listViewPhien.getSelectionModel().getSelectedIndex();
                listViewPhien.setItems(FXCollections.observableArrayList(pendingList));
                if (selectedIndex >= 0 && selectedIndex < pendingList.size()) {
    ...
    public void reviewAuction(MouseEvent event) {
        PhienDauGiaDTO phien = listViewPhien.getSelectionModel().getSelectedItem();

        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/ReviewPage.fxml"));
            StackPane homeRoot = loader.load();
            ReviewPageController reviewPageController = loader.getController();
            reviewPageController.setCurrentAuctionSession(phien);
            Scene homeScene = new Scene(homeRoot);
            Stage window = (Stage) ((Node) event.getSource()).getScene().getWindow();
            window.setScene(homeScene);
            window.show();
        } catch (IOException e) {
            HandleNavigationAndAlert.getInstance().showAlert(Alert.AlertType.ERROR, "Lỗi hiển thị", "Không hiển thị được review page");
            e.printStackTrace();
        }
    }
