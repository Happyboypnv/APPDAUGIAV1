...
public class FinanceAction {
    ...
    public void deposit(double amount) {
        try {
            ...
            // Gọi server với delta amount, không tự tính số dư mới ở client
            double newBalance = com.mycompany.utils.ApiClient.deposit(amount, token);
            if (newBalance < 0) {
                HandleNavigationAndAlert.getInstance().showAlert(
                    Alert.AlertType.ERROR, "Lỗi", "Không thể nạp tiền. Vui lòng thử lại!");
                return;
            }
            // Cập nhật object in-memory với số dư server trả về (chính xác)
            User currentUser = SessionManager.getInstance().getCurrentUser();
            if (currentUser != null) {
                currentUser.setActualBalance(newBalance);
            }
            HandleNavigationAndAlert.getInstance().showAlert(
            ...
            HandleNavigationAndAlert.getInstance().showAlert(
                Alert.AlertType.ERROR, "Lỗi thông tin", "Số tiền nạp không hợp lệ! Vui lòng nhập lại.");
        }
    }
    ...
    public void withdraw(double amount) {
        if (amount <= 0) {
            HandleNavigationAndAlert.getInstance().showAlert(
                Alert.AlertType.ERROR, "Lỗi thông tin", "Số tiền rút không hợp lệ! Vui lòng nhập lại.");
            return;
        }
        String token = SessionManager.getInstance().getServerToken();
        // Server tự kiểm tra available_balance (thực tế - frozen) và trừ atomically
        double newBalance = com.mycompany.utils.ApiClient.withdraw(amount, token);
        if (newBalance < 0) {
            HandleNavigationAndAlert.getInstance().showAlert(
                Alert.AlertType.ERROR, "Lỗi",
                "Số dư không đủ hoặc đang có tiền bị đóng băng do phiên đấu giá!");
            return;
        }
        // Cập nhật object in-memory với số dư server trả về
        User currentUser = SessionManager.getInstance().getCurrentUser();
        if (currentUser != null) {
            currentUser.setActualBalance(newBalance);
        }
