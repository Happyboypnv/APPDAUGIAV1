...
/**
 * Lớp BopasswordEncoder cung cấp các hàm để mã hóa, tạo salt và kiểm tra mật khẩu
...
public class PasswordEncoder {
    ...
    /**
     * HÀM: passwordEncoder(String password, String salt)
     ...
     *
     * @param password Mật khẩu gốc do người dùng nhập vào
     * @param salt Chuỗi salt đã được tạo từ hàm createSalt()
    ...
    /**
     * HÀM: checkPassword(String passwordIn, String passwordToCode, String salt)
     ...
     *
     * @param passwordIn Mật khẩu do người dùng nhập khi đăng nhập
     * @param passwordToCode Mật khẩu đã được mã hóa, lưu trữ trong database
     * @param salt Salt đã được lưu cùng với mật khẩu mã hóa
     * @return true nếu mật khẩu đúng, false nếu mật khẩu sai
     */
    public static boolean checkPassword(String passwordIn, String passwordToCode, String salt) {
        // Mã hóa mật khẩu vừa nhập bằng cùng salt mà đã sử dụng trước đó
        String hashedInput = passwordEncoder(passwordIn, salt);
        ...
        return hashedInput.equals(passwordToCode);
    }
}
