...
//                 server.broadcast(gson.toJson(event));
    ...
    // =========================================================
    // API: POST /api/users/deposit  →  nạp tiền (delta-based, thread-safe)
    // =========================================================
    /**
     * handleDeposit — Nạp tiền: server tự cộng delta vào DB, không để client tự tính số dư mới.
     *
     *   Method : POST
     *   URL    : /api/users/deposit
     *   Headers: Authorization: Bearer <token>
     *   Body   : { "amount": 500000.0 }
     *   Response 200: { "thongBao": "Nạp tiền thành công", "balance": <số dư mới> }
     *   Response 400: amount <= 0
     *   Response 401: thiếu / sai token
     */
    public synchronized void handleDeposit(HttpExchange exchange) throws IOException {
        if (!exchange.getRequestMethod().equalsIgnoreCase("POST")) {
            guiPhanHoi(exchange, 405, "{\"thongBao\":\"Chỉ chấp nhận POST\"}");
            return;
        }
        String email = authenticateAndGetEmail(exchange);
        ...
        String body = docBody(exchange);
        DepositWithdrawRequest req = gson.fromJson(body, DepositWithdrawRequest.class);
        if (req == null || req.amount <= 0) {
            guiPhanHoi(exchange, 400, "{\"thongBao\":\"Số tiền nạp phải lớn hơn 0\"}");
            return;
        }

        // Atomic increment trực tiếp trong DB
        String sql = "UPDATE nguoi_dung SET so_du_thuc_te = so_du_thuc_te + ? WHERE ma_nguoi_dung = ?";
        try (java.sql.PreparedStatement ps =
                 DatabaseConnection.getConnection().prepareStatement(sql)) {
            ps.setDouble(1, req.amount);
            ps.setString(2, nguoiDung.getUserId());
            ps.executeUpdate();
            DatabaseConnection.getConnection().commit();
        } catch (java.sql.SQLException e) {
            logger.error("[UserController] Lỗi deposit: " + e.getMessage());
            guiPhanHoi(exchange, 500, "{\"thongBao\":\"Lỗi server khi nạp tiền\"}");
            return;
        }

        User updated = khoNguoiDung.findByEmail(email);
        double newBalance = updated != null ? updated.getActualBalance() : nguoiDung.getActualBalance() + req.amount;
        logger.info("[UserController] ✅ Nạp {} cho {} → balance={}", req.amount, email, newBalance);
        guiPhanHoi(exchange, 200,
            "{\"thongBao\":\"Nạp tiền thành công\",\"balance\":" + newBalance + "}");
    }

    // =========================================================
    // API: POST /api/users/withdraw  →  rút tiền (delta-based, thread-safe)
    // =========================================================
    /**
     * handleWithdraw — Rút tiền: server kiểm tra available_balance rồi trừ, tránh race condition.
     *
     *   Method : POST
     *   URL    : /api/users/withdraw
     *   Headers: Authorization: Bearer <token>
     *   Body   : { "amount": 200000.0 }
     *   Response 200: { "thongBao": "Rút tiền thành công", "balance": <số dư mới> }
     *   Response 400: amount <= 0 hoặc không đủ số dư
     *   Response 401: thiếu / sai token
     */
    public synchronized void handleWithdraw(HttpExchange exchange) throws IOException {
        if (!exchange.getRequestMethod().equalsIgnoreCase("POST")) {
            guiPhanHoi(exchange, 405, "{\"thongBao\":\"Chỉ chấp nhận POST\"}");
            return;
        }
        ...
        DepositWithdrawRequest req = gson.fromJson(body, DepositWithdrawRequest.class);
        if (req == null || req.amount <= 0) {
            guiPhanHoi(exchange, 400, "{\"thongBao\":\"Số tiền rút phải lớn hơn 0\"}");
            return;
        }

        // Kiểm tra available (thực tế - frozen) TRONG DB để tránh race condition
        double availableBalance = nguoiDung.getActualBalance() - nguoiDung.getFrozenBalance();
        if (req.amount > availableBalance) {
            guiPhanHoi(exchange, 400,
                "{\"thongBao\":\"Số dư khả dụng không đủ. Khả dụng: " + availableBalance + "\"}");
            return;
        }

        // Atomic decrement trong DB (chỉ trừ so_du_thuc_te vì frozen chưa thực sự rút)
        String sql = "UPDATE nguoi_dung SET so_du_thuc_te = so_du_thuc_te - ? WHERE ma_nguoi_dung = ? AND (so_du_thuc_te - so_du_dong_bang) >= ?";
        try (java.sql.PreparedStatement ps =
                 DatabaseConnection.getConnection().prepareStatement(sql)) {
            ps.setDouble(1, req.amount);
            ps.setString(2, nguoiDung.getUserId());
            ps.setDouble(3, req.amount);
            int rows = ps.executeUpdate();
            DatabaseConnection.getConnection().commit();
            if (rows == 0) {
                guiPhanHoi(exchange, 400, "{\"thongBao\":\"Số dư không đủ sau khi kiểm tra lại (concurrent)\"}");
                return;
            }
        } catch (java.sql.SQLException e) {
            logger.error("[UserController] Lỗi withdraw: " + e.getMessage());
            guiPhanHoi(exchange, 500, "{\"thongBao\":\"Lỗi server khi rút tiền\"}");
            return;
        }
    ...
    private String authenticateAndGetEmail(HttpExchange exchange) throws IOException {
        String authHeader = exchange.getRequestHeaders().getFirst("Authorization");
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            guiPhanHoi(exchange, 401, "{\"thongBao\":\"Cần đăng nhập trước\"}");
            return null;
